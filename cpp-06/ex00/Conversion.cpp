...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/16 13:08:05 by ltalia            #+#    #+#             */
/*   Updated: 2022/11/16 13:09:55 by ltalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	Conversion::checkInput()
{
		...
		this->getInput().compare("-inff") == 0)
	{
		...
		(this->getInput()[0] == '+' || this->getInput()[0] == '-' || // nterpreted as a char
		this->getInput()[0] == 'f' || this->getInput()[0] == '.'))
	{
		return (CHAR);
	}
	else if (this->getInput().find_first_of("+-") != this->getInput().find_last_of("+-")) // catches any mixed use of + and -
		return (ERROR);
